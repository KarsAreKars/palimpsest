...
PAGE IMAGES: you also receive rendered image(s) of the visible page(s). Use them as evidence for figures, diagrams, layout, and how equations actually render. The images are evidence, NOT the addressing system — your marks still reference block ids only. If the image and the text excerpt disagree, trust the image for what the page looks like and the text layer for block ids.
`;
