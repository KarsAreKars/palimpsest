...
- **Function plots are CORE, not v1.1** (owner ruling 2026-09-16): visualizing
  ...
  — LaTeX in (MathLive Compute Engine evaluates it offline), rendered with
  **Mafs** (React interactive math viz, bundled — offline, no embed): pan,
  zoom, parameter sliders, draggable points; restyled to the paper world
  (ink axes, stamp curve). Static SVG only where interactivity adds nothing.
  The professor can sketch the parabola beside `y = a·x²` — and the learner
  grabs the slider. Ships in the pivot's final wave (reuses cluster placement).
