...
function view() {
  return document.body.dataset.view || 'grid';
}
...
function setView(v) {
  ...
  document
    .querySelectorAll('[data-setview]')
    .forEach((b) => b.setAttribute('aria-pressed', String(b.dataset.setview === v)));
...
function show(i) {
  ...
  document
    .querySelectorAll('.note-group')
    .forEach((g) => g.classList.toggle('active', g.dataset.for === frames[cur].id));
  syncHash();
  fit();
}
...
function fit() {
  ...
  if (!stage) return;
  const rail =
    view() === 'slides' ? f.closest('.board-section')?.querySelector('.note-rail') : null;
  ...
  const pageCentered = stage.clientWidth / 2 + (f.offsetWidth * s) / 2 <= stage.clientWidth - railW;
...
document
  .querySelectorAll('[data-setview]')
  .forEach((b) => b.addEventListener('click', () => setView(b.dataset.setview)));
prevBtn?.addEventListener('click', () => (view() === 'proto' ? histBack() : show(cur - 1)));
...
let saved = null;
try {
  saved = localStorage.getItem('wf-theme');
} catch (e) {}
...
document
  .querySelectorAll('.wf .chip')
  .forEach((c) => c.addEventListener('click', () => c.classList.toggle('active')));
document
  .querySelectorAll('.wf .switch')
  .forEach((s) =>
